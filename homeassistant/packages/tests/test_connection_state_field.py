...
def test_manifest_count_is_thirty_four(manifest_count: int) -> None:
    ...
    33rd manifest and Wave 9 #123.b adds `connections/factory-reset/`
    as the 34th. If a future slice adds a new connection, update
    ...
    assert manifest_count == 34, (
        ...
        f"#123.b factory-reset); got {manifest_count}. The slice's "
        f"primary value is uniform data, not bespoke configuration "
        f"— adding a connection is a deliberate action."
    )
