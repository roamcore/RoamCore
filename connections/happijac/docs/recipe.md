# Happijac bed lift — tier-b recipe connection

**Tier:** B (recipe)
**Audience:** A RoamCore user who wants vendor-neutral up/down control
of a Happijac (or any 2-relay + 2-limit-switch bed lift: LCI Happijac,
DIY linear actuators, winch + strap, etc.) system with safety-first
wiring interlocks, using one of two upstream operator-side paths:
ESPHome custom `cover:` (Path A — recommended for ESPHome-friendly
installs), or a Shelly 1 / Shelly Plus 1 / Zooz ZEN17 / Aeotec Nano
Switch relay pair + HA core `template:` cover (Path B — recommended
for relay-friendly installs). Both paths land on the same
vendor-neutral contract layer via `rc_bed_lift_*` dashboard tiles +
OpenClaw bed-lift queries ("lift the bed", "lower the bed", "stop
the bed", "what's the bed position?", "is the bed safe?", "is the
bed moving?", "is the bed obstructed?", "is the bed low-voltage
locked?", "set bed mode").

This howto is mirrored into `docs/connections/happijac.md` by the
...
- **Vendor-neutral cover semantics.** RoamCore wants a `cover.<name>`
  tile (not a vendor-specific `cover.happijac` tile) so that a
  Happijac today + a DIY linear-actuator bed tomorrow + a winch + strap
  build next year all drop onto the same contract layer. The cover
  template `cover.rc_bed_lift_position` is the single integration
  point — every automation, every OpenClaw query, every dashboard
  tile binds to that contract entity.
- **Two upstream paths** (operator picks based on existing IoT
  ...
  - **Path A — ESPHome custom `cover:`** (recommended for
    ...
    declare `cover.bed_lift` (NOT `cover.happijac_bed_lift`) so the
    entity id is operator-neutral. ESPHome exposes a config_flow since
    2023.x.
  - **Path B — Dry-contact relay + HA core `template:` cover** (no
    ...
    are wired to the Happijac controller's up/down inputs (5 V signal,
    common, up, down). HA auto-discovers the Shelly units via mDNS
    (`shelly` integration has a config_flow since 2019.x); the upstream
    `switch.shelly_*_relay` entities + the upstream
...
RoamCore ships no native Happijac controller. We RECIPE the well-
...
- **Happijac bed lift system** (or any 2-relay + 2-limit-switch bed
  lift: LCI Happijac, DIY linear actuators, winch + strap, etc.).
  Either the stock LCI Happijac controller OR a third-party controller
  with up/down dry-contact inputs that a relay pair can drive.
- **2× dry-contact relays** with the coil voltage matched to the
  ...
  - **Path B (Shelly / Shelly Plus / Zooz ZEN17 / Aeotec Nano
    ...
    (Zooz ZEN17). Pick the variant that matches the Happijac
    controller's coil voltage.
...
- **2× limit microswitches** (NC or NO per the Happijac controller's
  wiring) with mechanical debounce. Both microswitches MUST be wired
  into either the ESPHome binary_sensor inputs (Path A) or the
  Shelly dry-contact inputs (Path B). The recipe assumes NO (normally
  open) microswitches wired to the ESPHome / Shelly pull-up — the
  NC variant inverts the binary_sensor pull-up logic and is
  documented in the §8 troubleshooting entry "bed moves up but not
  down (NC/NO mis-wire)".
...
- The operator MUST understand the wiring of their Happijac
  controller's up/down dry-contact inputs (the LCI Happijac manual
  is the reference; for DIY controllers the manual). No RoamCore
...
```yaml
# connections/happijac/bed-lift.yaml (Path A ESPHome device)
esphome:
  name: roamcore-bed-lift
  friendly_name: RoamCore Bed Lift
  platform: ESP32
  board: esp32dev
...
wifi:
  ...
  ap:
    ssid: "RoamCore Bed Lift Fallback"
...
```yaml
# connections/happijac/cover-template.yaml (Path A — wraps the
...
- **Shelly 1 unit 1 (UP relay)**:
  - L (line) → the Happijac controller's UP coil +V common
  - N (neutral) → the Happijac controller's UP coil neutral
  - The Shelly output dry-contact (O) → the Happijac controller's UP
    dry-contact input
...
- **Shelly 1 unit 2 (DOWN relay)**:
  - L (line) → the Happijac controller's DOWN coil +V common
  - N (neutral) → the Happijac controller's DOWN coil neutral
  - The Shelly output dry-contact (O) → the Happijac controller's
    DOWN dry-contact input
  - 5 V signal → the DOWN limit microswitch common
  - The Shelly dry-contact input (SW) → the DOWN limit microswitch
    NO contact
...
- `binary_sensor.shelly_bed_lift_down_dry_contact` (DOWN limit microswitch)
...
```yaml
# connections/happijac/cover-template-path-b.yaml (Path B — wraps
# the upstream `switch.shelly_bed_lift_up_relay` +
...
```yaml
# connections/happijac/contract-tiles.yaml (template — both
...
Six automations ship as copy-pasteable YAML; the operator imports
...
operator's specific Happijac motor (a lighter-duty Happijac motor
might have a normal-startup inrush spike >5 A; a heavier-duty
Happijac motor has a different baseline current draw).
...
The Happijac bed lift produces no telemetry beyond the limit
...
Neither the Happijac controller nor the upstream integrations call
home to a cloud service — there is no Happijac cloud, no ESPHome
cloud, no Shelly cloud for relay control (Shelly's cloud is purely
optional + off by default), no Zooz / Aeotec cloud for relay
control.
...
A real Happijac + 2× relay bench on CI (a bench with at least one
real Happijac + 2× dry-contact relays + 2× limit microswitches +
optional CT-clamp current sensor + an ESP32 flashed with the recipe
§3 ESPHome YAML + at least one Shelly / Shelly Plus / Zooz / Aeotec
unit wired into the bench's no-load Happijac simulator) is required
to flip this connection from tier-b to tier-a.

What tier-a promotion requires:

1. **A real Happijac + ESPHome + relay bench on CI.** The bench
   needs at least one ESP32 flashed with the recipe §3 ESPHome
   YAML + two Shelly / Shelly Plus / Zooz ZEN17 / Aeotec Nano
   Switch units wired into the bench's no-load Happijac simulator
   + a CT-clamp current sensor wired into an ADC pin. The CI
   bench drives the ESPHome device over its API to simulate bed
...
3. **Integration tests** that assert:
   ...
   - A `binary_sensor.rc_bed_lift_safety_ok = FALSE` scenario (simulated
     both-limits-TRUE wiring fault) blocks any subsequent motion
     command.
   - A `binary_sensor.rc_bed_lift_low_voltage_lockout = TRUE` scenario
     (simulated SOC < 20 %) blocks any subsequent motion command.
...
RoamCore code, not just documented in the recipe), and the connection
flips to `tier: a` with `wizard.one_tap: true` +
...
Until that bench lands, this connection stays at tier-b. The recipe
is sound; the safety interlocks are documented; the contract tiles
work; but we cannot claim one-tap automation without bench
