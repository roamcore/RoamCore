...
LEGACY_DOC = REPO_ROOT / "docs" / "catalog" / "time" / "atomic-time.md"
...
def test_category_matches_existing_legacy_doc(manifest: dict) -> None:
    ...
    docs/catalog/time/atomic-time.md (created in this slice as
    the legacy spec pointer — mirrors how Wave 3 #54
    timezone-geolocator promoted docs/catalog/time/timezone-
    geolocator.md). We promote the connection into the `time`
    category so the audit + boundary-CI can pair them up. The
    legacy doc MUST still exist (with the supersession banner)
    so that the recipe can reference it AND the audit can
    verify the supersession banner is in place.
    """
    assert manifest["category"] == "time", (
        f"category must stay 'time' (legacy doc lives at "
        f"docs/catalog/time/atomic-time.md); got "
    ...
    legacy_text = LEGACY_DOC.read_text(encoding="utf-8")
    assert "Moved" in legacy_text and "connections/time-atomic/docs/recipe.md" in legacy_text, (
        "legacy docs/catalog/time/atomic-time.md must be a 2-line 'Moved to ...' redirect page pointing at "
        "connections/time-atomic/docs/recipe.md (Wave 9 #124c); got:\n" + legacy_text[:200]
    )
    ...
    assert "SUPERSEDED" not in legacy_text, (
        "legacy docs/catalog/time/atomic-time.md must not carry the 'SUPERSEDED' banner (Wave 9 "
