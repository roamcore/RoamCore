...
def main() -> int:
    for catdir in sorted(CAT.iterdir()):
        ...
        # Wave 9 #124c: skip "Moved to ..." redirect pages so the
        # autogenerated feature list doesn't clobber the canonical
        # redirect content (these pages are thin 2-line "Moved to
        # connections/<slug>/docs/recipe.md/" pointers that exist
        # for backward-link compatibility AND as mkdocs nav landing
        # pages — clobbering them with an empty feature list breaks
        # both). Detected by "# Moved" first H1.
        existing = read(idx)
        if existing.lstrip().startswith("# Moved"):
            continue
