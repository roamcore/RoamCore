...
#   bash scripts/check.sh --core-only    # repo-local checks (HA beta + networking-controls + remote-access smokes)
#
# Slices wired in:
#   ▶ HA-only beta: smoke check
#   ▶ Networking controls (OpenWrt API): smoke check   (slice #28 — sibling branch)
#   ▶ Remote access (Tailscale contract): smoke check   (slice #29 — this commit)
#
...
# Networking controls (OpenWrt API) smoke ships on sibling branch
# feat/wave2-networking-controls-openwrt (slice #28). Until that PR merges,
# probe so this branch's chain stays green on main.
run_if_present "scripts/checks/openwrt-controls-smoke.sh" \
  "Networking controls (OpenWrt API): smoke check"

banner "Remote access (Tailscale contract): smoke check"
bash scripts/checks/remote-access-tailscale-smoke.sh
