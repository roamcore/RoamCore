...
#   bash scripts/check.sh --core-only    # repo-local checks (HA beta + mode builder)
#
...
# Slice #21 (trip wrapped — seamless USP flow) wires these on top of main:
#   ▶ Trip wrapped seamless: smoke check
# Slice #20 (trip tracking — local/private) wires these on top of main:
