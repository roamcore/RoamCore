...
# 4c. Traccar install + integration smoke check (Wave 2 #17).
if [ -f scripts/checks/traccar-integration-smoke.sh ]; then
    run_check "Traccar: install + integration smoke check" \
...
# 4e. HACS packaging smoke check (Wave 2 #19).
if [ -f scripts/checks/hacs-packaging-smoke.sh ]; then
    run_check "HACS: packaging smoke check" \
        "bash scripts/checks/hacs-packaging-smoke.sh"
fi
