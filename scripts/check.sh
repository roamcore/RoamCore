...
#   bash scripts/check.sh --core-only    # repo-local checks (HA beta + networking-controls smoke)
#
# Slices wired in:
#   ▶ HA-only beta: smoke check
#   ▶ Networking controls (OpenWrt API): smoke check   (slice #28 — this commit)
#
...
banner "Networking controls (OpenWrt API): smoke check"
bash scripts/checks/openwrt-controls-smoke.sh
