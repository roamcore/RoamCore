...
- `mqtt` — vendor-neutral lightweight pub/sub messaging layer that the upstream HA core `mqtt` integration uses for IoT devices (broker-online chip + broker-status + broker-url + discovery-count + reconnect-now button + master enable + broker-username + tls-enabled tiles); vendor-neutral `rc_mqtt_*` ids (Wave 3 #34)
...
- `mqtt` — vendor-neutral lightweight pub/sub messaging layer for IoT devices (broker-online chip + broker-status + broker-url + discovery-count + reconnect-now button + master enable + broker-username + tls-enabled tiles). OWNED by `connections/mqtt/` (Wave 3 #34).
